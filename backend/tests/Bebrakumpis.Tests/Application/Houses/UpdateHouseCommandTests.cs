...
public class UpdateHouseCommandTests
{
    ...
    {
        var id = Guid.NewGuid();
        var house = new House { Id = id, Name = "Old Name", BookingColor = "#000000", ReservedColor = "#ffffff", CreatedAt = DateTime.UtcNow };
        _repoMock.Setup(r => r.GetByIdAsync(id, default)).ReturnsAsync(house);

        var result = await _handler.HandleAsync(
            new UpdateHouseCommand(id, "New Name", "#3b82f6", "#ef4444"), default);
    ...
    public async Task HandleAsync_ShouldReturnNotFound_WhenHouseDoesNotExist()
    {
        ...
        var result = await _handler.HandleAsync(
            new UpdateHouseCommand(Guid.NewGuid(), "Name", "#3b82f6", "#ef4444"), default);
    ...
    {
        var result = await _handler.HandleAsync(
            new UpdateHouseCommand(Guid.NewGuid(), "Name", "bad", "#ef4444"), default);
